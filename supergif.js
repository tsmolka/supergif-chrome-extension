...
(function (root, factory) {
  ...
  var SuperGif = function ( opts ) {
    ...
    return {
      ...
      register_canvas_handers: function () {
        var sup = this;
        ...
        canvas.addEventListener((cantouch) ? 'touchmove' : 'mousemove', function (e) {
          ...
          if (startTime !== 0 && (currentTime - startTime < maxTime || isDragging) && currentDistance > maxDistance) {
            isDragging = true;
...
// Listen to message from service-worker.js
chrome.runtime.onMessage.addListener((msg) => {
  if (!msg.srcUrl)
    return;
  
  // Enumerate all images in current document
  var imgs = document.getElementsByTagName('img');
  for (var i = 0; i < imgs.length; i++) {
    var img = imgs[i];

    // Check image source
    if (msg.srcUrl !== img.src) {
      continue;
    }

    // Initialize SuperGif
    var sup = new SuperGif({
      gif: img,
      max_width: img.width,
      draw_while_loading: true,
      show_progress_bar: true,
      progressbar_background_color: '#ffffff',
      progressbar_foreground_color: '#000000',
